...
import server.clip.VariableObject;
import server.game.objects.*;
...
public class Region {
 ...
 private ArrayList<Objects> realObjects = new ArrayList<Objects>();
	...
	public static Hashtable<Integer, VariableObject> variableObjects = new Hashtable<Integer, VariableObject>();
	 public static int getRotation(int x, int y/*, int height*/) {
	    	VariableObject o = variableObjects.get(y + (x << 16));
	    	if(o == null) return 0;
	    	return o.getFace();
	    }
 ...
 return true;
    ...
    public static void loadClipTiles() {
    	addClipping(3093, 3499, 0, 256);
    }
    public static void load() {
        ...
        for (int i = 0; i < size; i++) {
            regions[i] = new Region(regionIds[i], true);
        }
        for (int i = 0; i < size; i++) {
