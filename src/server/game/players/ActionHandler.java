...
import server.game.players.content.dialogue.DialogueManTest;
...
public class ActionHandler {
	...
	public void firstClickObject(int objectType, int obX, int obY) {
		c.clickObjectType = 0;
		// if(c.playerRights == 3)
		// System.out.println(Region.getRotation(obX, obY));
		// Woodcutting.startChopping(c, objectType, obX, obY);
		// Doors.openDoor(c, objectType, obX, obY);

		if (c.playerRights == 3)
			System.out.println("[" + c.playerName
					+ "] first obj click is " + objectType + " x = " + obX
					+ " y = " + obY + "\n");
		...
		case 21600:
			...
			break;
		case 21507:
			c.getPA().makeGlobalObject(21507, obX, obY, 1, 0);
			break;
		case 21505:
			c.getPA().makeGlobalObject(21505, obX, obY, 3, 0);
			break;
		case 21340:
			c.getPA().makeGlobalObject(21340 	, obX, obY, 2, 0);
			break;
		case 21341:
			if(obX ==2347 && obY ==3801 )
				c.getPA().makeGlobalObject(21341 	, obX, obY, 2, 0);
			else
				c.getPA().makeGlobalObject(21341 	, obX, obY, 1, 0);
			break;
		case 24381:
			if (obX == 3079 && obY == 3497) {
				c.getPA().makeGlobalObject(24381, obX, obY, 2, 0);

			} else if (obX == 3255 && obY == 3388) {
				c.getPA().makeGlobalObject(24381, obX, obY, 3, 0);

			} else if (obX == 3235 && obY == 3404) {
				c.getPA().makeGlobalObject(24381, obX, obY, 3, 0);
			}
		...
		case 2148:
			...
			break;
		case 11993:
			if (obX == 3109 && obY == 3167) {
				c.getPA().checkObjectSpawn(11993, obX, obY, 4, 0);
			}
			if (obX == 3111 && obY == 3162) {
				c.getPA().checkObjectSpawn(11993, obX, obY, 4, 0);
			}
			if (obX == 3107 && obY == 3162) {
				if (c.absX == 3107 && c.absY == 3163) {
					c.getPA().movePlayer(3106, 3162, 0);
				} else if (c.absX == 3106 && c.absY == 3162) {
					c.getPA().movePlayer(3107, 3162, 0);
				}
			}
			break;
		case 2025:
			if (c.playerLevel[c.playerFishing] > 60)
				c.getPA().checkObjectSpawn(-1, obX, obY, 3, 0);
			else
				c.sendMessage("Your fishing level must be higher than 60");
			break;
		case 1536: // open door
			...
			if (obX == 3215 && obY == 3211) {
				c.getPA().checkObjectSpawn(1530, obX, obY, 4, 0);
			}
			if (obX == 3207 && obY == 3222) {
				c.getPA().checkObjectSpawn(1530, obX, obY, 3, 0);
			}
		...
		case 9301:
			if (c.absX == 2575 && c.absY == 3107 && c.playerLevel[16] >= 65) {
				...
				c.getPA().addSkillXP(100, c.playerAgility);
				c.getPA().refreshSkill(c.playerAgility);
				c.turnPlayerTo(c.getX() - 1, c.getY());
			} else {
				c.sendMessage("You need 65 Agility to pass trough...");
			}
			break;

		case 9302:
			if (c.absX == 2575 && c.absY == 3112 && c.playerLevel[16] >= 65) {
				c.startAnimation(746);
				c.teleportToX = 2575;
				c.teleportToY = 3107;
				c.heightLevel = 0;
		...
		case 2113:
			if (obX == 3020 && obY == 3339) {
				c.getPA().movePlayer(3021, 9739, 0);
			}
			if (obX == 3019 && obY == 3338) {// listo
				c.getPA().movePlayer(3019, 9737, 0);
			}
			if (obX == 3018 && obY == 3339) {// listo
				c.getPA().movePlayer(3017, 9739, 0);
			}
			if (obX == 3019 && obY == 3340) {
				c.getPA().movePlayer(3019, 9741, 0);
			}
			break;
		// arriba derecha 3021 3339
		// arriba arriba 3019 3341
		// abajo derecha 3021 9739
		// abajo arriba 3019 9741
		...
		case 1755:
			if (obX == 3097 && obY == 9867) {
				c.getPA().movePlayer(3097, 3468, 0);
			}
			if (obX == 3020 && obY == 9739) {
				c.getPA().movePlayer(3021, 3339, 0);
			}
			if (obX == 3019 && obY == 9738) {// listo
				c.getPA().movePlayer(3019, 3337, 0);
			}
			if (obX == 3018 && obY == 9739) {// listo
				c.getPA().movePlayer(3017, 3339, 0);
			}
			if (obX == 3019 && obY == 9740) {
				c.getPA().movePlayer(3019, 3341, 0);
			}
			if (obX == 3116 && obY == 9852) {
				c.getPA().movePlayer(3116, 3451, 0);
			}
			break;

		case 9325:
			if (obX == 3034 && obY == 9806 && c.playerLevel[16] >= 65) {
				c.startAnimation(1110);
			...
			} else if (obX == 3034 && obY == 9806 && c.playerLevel[16] <= 65) {
				c.sendMessage("You need 65 Agility to pass trough...");
			}
			if (obX == 3029 && obY == 9806 && c.playerLevel[16] >= 65) {
				c.startAnimation(1110);
				Server.getTaskScheduler().schedule(new Task(2, false) {
			...
			} else if (obX == 3029 && obY == 9806 && c.playerLevel[16] <= 65) {
				c.sendMessage("You need 65 Agility to pass trough...");
			}
			break;
		...
		case 5083:
			...
			c.sendMessage("You enter the Brimhaven dungeon.");
			break;
		case 2492:
			if (obX == 2889 && obY == 4813 || obX == 2885 && obY == 4850
					|| obX == 2932 && obY == 4854 || obX == 2933 && obY == 4815)
				c.getPA().movePlayer(3252, 3402, 0);
			break;
		case 6238:
		case 6240:
		...
		case 9311: // grand exchange agility cave
			if (c.absX == 3187 && c.absY == 3493 && c.playerLevel[16] >= 65) {
				c.startAnimation(746);
				c.teleportToX = 3193;
				c.teleportToY = 3493;
				c.heightLevel = 0;
		...
		case 9312: // grand exchange agility cave
			if (c.absX == 3193 && c.absY == 3493 && c.playerLevel[16] >= 65) {
				c.startAnimation(746);
				c.teleportToX = 3187;
				c.teleportToY = 3493;
				c.heightLevel = 0;
		...
		case 17010:
			...
			break;
		case 4250:
			if (obX == 2627 && obY == 3675) {
				c.getPA().checkObjectSpawn(4250, obX, obY, 1, 0);
			}

		case 102:
			if (obX == 3176 && obY == 3481) {
				c.getPA().checkObjectSpawn(102, obX, obY, 2, 0);
			}

		case 7050:

			// cows Falador
			if (obX == 3032 && obY == 3313) {
				c.getPA().checkObjectSpawn(7049, obX, obY, 2, 0);
			}
			if (obX == 3021 && obY == 3306) {
				c.getPA().checkObjectSpawn(7049, obX, obY, 1, 0);
			}
			if (obX == 3021 && obY == 3301) {
				c.getPA().checkObjectSpawn(7049, obX, obY, 1, 0);
			} else {
				c.getPA().makeGlobalObject(-1, obX, obY, 0, 0);
			}
		...
		case 1508:
			// cows Falador
			if (obX == 3031 && obY == 3313) {
				c.getPA().checkObjectSpawn(7050, obX, obY, 4, 0);
			}
			if (obX == 3021 && obY == 3305) {
				c.getPA().checkObjectSpawn(7050, obX, obY, 3, 0);
			}
			if (obX == 3021 && obY == 3300) {
				c.getPA().checkObjectSpawn(7050, obX, obY, 3, 0);
			} else {
				c.getPA().makeGlobalObject(-1, obX, obY, 0, 0);
			}
		...
		case 1551:
			...
			if (obX == 3253 && obY == 3266) {
				c.getPA().checkObjectSpawn(1553, obX, obY, 3, 0);
			}

			// chickens
			if (obX == 3236 && obY == 3296) {
				c.getPA().checkObjectSpawn(1553, obX, obY, 1, 0);
			}

			// potaotes
			if (obX == 3241 && obY == 3301) {
				c.getPA().checkObjectSpawn(1551, 3240, obY, 3, 0);
				c.getPA().checkObjectSpawn(-1, obX, obY, 3, 0);
			}
			// Wheat
			if (obX == 3163 && obY == 3290) {
				c.getPA().checkObjectSpawn(1553, obX, obY, 2, 0);
			}
		...
		case 1553:
			...
			if (obX == 3253 && obY == 3267) {
				c.getPA().checkObjectSpawn(1551, obX, obY, 1, 0);
			}
			// chickens
			if (obX == 3236 && obY == 3295) {
				c.getPA().checkObjectSpawn(1551, obX, obY, 3, 0);
			}
			// potaotes
			if (obX == 3241 && obY == 3302) {
				c.getPA().checkObjectSpawn(1553, 3240, obY, 1, 0);
				c.getPA().checkObjectSpawn(-1, obX, obY, 3, 0);
			}
			// Wheat
			if (obX == 3162 && obY == 3290) {
				c.getPA().checkObjectSpawn(1551, obX, obY, 4, 0);
			}
			break;

		case 1804:
			// hill giants
			if (obX == 3115 && obY == 3450 && c.getItems().playerHasItem(983)) {
				c.getPA().checkObjectSpawn(1530, obX, obY, 4, 0);
			} else
				c.sendMessage("You need a Brass Key");
			break;
		...
		case 26933:
			if (obX == 3097 && obY == 3468) {
				c.getPA().movePlayer(3097, 9869, 0);
			}
			break;
		...
		case 1516:
			if (c.absX == 3217 && c.absY == 3219 || c.absX == 3217
					&& c.absY == 3218) {
				c.getPA().movePlayer(3216, 3218, 0);
			}
			if (c.absX == 3216 && c.absY == 3219 || c.absX == 3216
					&& c.absY == 3218) {
				c.getPA().movePlayer(3217, 3218, 0);
			}
			if (c.absX == 3166 && c.absY == 3302 || c.absX == 3167
					&& c.absY == 3302) {
				c.getPA().movePlayer(3166, 3303, 0);
			}
			if (c.absX == 3166 && c.absY == 3303 || c.absX == 3167
					&& c.absY == 3303) {
				c.getPA().movePlayer(3166, 3302, 0);
			}
			if (obX == 3244 && obY == 3216) {
				c.sendMessage("You cant open this door..");
			}
			break;
		...
		case 1519:
			if (c.absX == 3217 && c.absY == 3219 || c.absX == 3217
					&& c.absY == 3218) {
				c.getPA().movePlayer(3216, 3219, 0);
			}
			if (c.absX == 3216 && c.absY == 3219 || c.absX == 3216
					&& c.absY == 3218) {
				c.getPA().movePlayer(3217, 3219, 0);
			}
			if (c.absX == 3166 && c.absY == 3302 || c.absX == 3167
					&& c.absY == 3302) {
				c.getPA().movePlayer(3167, 3303, 0);
			}
			if (c.absX == 3166 && c.absY == 3303 || c.absX == 3167
					&& c.absY == 3303) {
				c.getPA().movePlayer(3167, 3302, 0);
			}
			if (obX == 3243 && obY == 3216) {
				c.sendMessage("You cant open this door..");
			}
			break;

		case 1568:
			if (obX == 3217 && obY == 3219) {
				c.getPA().makeGlobalObject(1568, obX, obY, 2, 0);
			}

		case 12349:
			if (c.absX == 3212 && c.absY == 3221 || c.absX == 3212
					&& c.absY == 3222) {
				c.getPA().movePlayer(c.absX + 1, obY, 0);
			}
			if (c.absX == 3213 && c.absY == 3221 || c.absX == 3213
					&& c.absY == 3222) {
				c.getPA().movePlayer(c.absX - 1, obY, 0);
			}
			break;
		...
		case 134:
			if (c.heightLevel > 3) {
				if (c.getItems().playerHasItem(9722)) {
					// c.getItems().deleteItem(9722, 1);
				} else {
					c.sendMessage("You need a key to pass through, it wont take it away");
					return;
				}
			}
			if (c.absX == 3109 && c.absY == 3353 || c.absX == 3108
					&& c.absY == 3353) {
				c.getPA().movePlayer(3108, 3354, 0);
			}
			if (c.absX == 3109 && c.absY == 3354 || c.absX == 3108
					&& c.absY == 3354) {
				c.getPA().movePlayer(3108, 3353, 0);
			}
			break;
		...
		case 135:
			...
			if (c.absX == 3109 && c.absY == 3354 || c.absX == 3108
					&& c.absY == 3354) {
				c.getPA().movePlayer(3109, 3353, 0);
			}
			if (c.absX == 3109 && c.absY == 3353 || c.absX == 3108
					&& c.absY == 3353) {
		...
		case 136:
			if (obX == 3123 && obX == 3361) {
				c.getPA().checkObjectSpawn(136, obX, obY, 2, 0);
			}
			break;

		case 155:
			if (c.absX == 3098 && c.absY == 3358) {
				c.getPA().movePlayer(obX - 1, obY, 0);
			}
			if (c.absX == 3096 && c.absY == 3358) {
				c.getPA().movePlayer(obX + 1, obY, 0);
			}
			break;

		case 156:
			if (c.absX == 3098 && c.absY == 3359) {
				c.getPA().movePlayer(obX - 1, obY, 0);
			}
			if (c.absX == 3096 && c.absY == 3359) {
				c.getPA().movePlayer(obX + 1, obY, 0);
			}
			break;
		...
		case 11470:
			if (c.heightLevel > 3) {
				if (c.getItems().playerHasItem(9722)) {
					...
					c.sendMessage("You need a key to open.");
					return;
				}
			}
			if (obX == 3109 && obY == 3358) {
				c.getPA().checkObjectSpawn(11470, obX, obY, 2, 0);
			}
			if (obX == 3106 && obY == 3368) {
				c.getPA().checkObjectSpawn(11470, obX, obY, 2, 0);
			}
			if (obX == 3101 && obY == 3371) {
				c.getPA().checkObjectSpawn(11470, obX, obY, 1, 0);
			}
			if (obX == 3103 && obY == 3364) {
				c.getPA().checkObjectSpawn(11470, obX, obY, 2, 0);
			}
			if (obX == 3099 && obY == 3366) {
				c.getPA().checkObjectSpawn(11470, obX, obY, 2, 0);
			}
			if (obX == 3120 && obY == 3356) {
				c.getPA().checkObjectSpawn(11470, obX, obY, 1, 0);
			}
			break;

		case 15514:
			if (obX == 3080 && obY == 3501) {
				c.getPA().checkObjectSpawn(15514, obX, 3500, 2, 0);
				c.getPA().checkObjectSpawn(-1, obX, obY, -1, 0);
			}
			break;

		case 9299:
			if (obX == 3240 && obY == 3190) {
				c.getPA().checkObjectSpawn(9299, obX, obY, 4, 0);
			}
			break;

		case 15516:
			if (obX == 3079 && obY == 3501) {
				c.getPA().checkObjectSpawn(15516, obX, 3500, 4, 0);
				c.getPA().checkObjectSpawn(-1, obX, obY, 4, 0);
			}
			break;

		case 24376:
			if (obX == 3205 && obY == 3432) {
				c.getPA().makeGlobalObject(24376, obX, obY, 3, 0);
			}
			break;
		...
		case 15536:
			...
			if (obX == 3215 && obY == 3477) { // north
				c.getPA().makeGlobalObject(3215, 3477, 6951, 0, 0);
				c.getPA().makeGlobalObject(3215, 3477, 1537, 0, 0);
			}
			...
			if (obX == 3258 && obY == 3485) {
				c.getPA().checkObjectSpawn(15536, obX, 3484, 4, 0);
				c.getPA().checkObjectSpawn(-1, obX, obY, 4, 0);
			}
			if (obX == 3215 && obY == 3477) {
				c.getPA().checkObjectSpawn(15536, obX, obY, 2, 0);
			}
			break;

		case 1530:
			if (obX == 3108 && obY == 9570) {
				c.getPA().checkObjectSpawn(1530, obX, obY, 3, 0);
			}
			if (obX == 3234 && obY == 3156) {
				c.getPA().checkObjectSpawn(1530, obX, obY, 4, 0);
			}
			if (obX == 3246 && obY == 3244) {
				c.getPA().checkObjectSpawn(1530, obX, obY, 2, 0);
			} else if (obX == 3070 && obY == 3515) {
				c.getPA().checkObjectSpawn(1530, obX, obY, 2, 0);
			}
			if (obX == 3208 && obY == 3211) {
				c.getPA().checkObjectSpawn(1530, obX, obY, 4, 0);
			}
			if (obX == 3247 && obY == 3193) {
				c.getPA().checkObjectSpawn(1530, obX, obY, 3, 0);
			}
			if (obX == 2884 && obY == 3423) {
				c.getPA().checkObjectSpawn(1530, obX, obY, 3, 0);
			} else {
				c.getPA().makeGlobalObject(-1, obX, obY, 3, 0);
			}
			break;
		case 1531:
		case 1533:
		...
		case 3197:
			...
			break;
		case 11707:
			if (obX == 2949 && obY == 3379) {
				c.getPA().checkObjectSpawn(11707, 2948, obY, 1, 0);
				c.getPA().checkObjectSpawn(-1, obX, obY, 1, 0);
			}
			if (obX == 3061 && obY == 3374) {
				c.getPA().checkObjectSpawn(11707, obX, 3375, 2, 0);
				c.getPA().checkObjectSpawn(-1, obX, obY, 1, 0);
			}
		...
		case 1558:
			if (c.absX == 3041 && c.absY == 10308) {
				...
				c.getPA().movePlayer(3045, 10342, 0);
			} else if (c.absX == 3145 && c.absY == 9870 || c.absX == 3145
					&& c.absY == 9870) {
				c.getPA().movePlayer(3146, 9870, 0);
			} else if (c.absX == 3146 && c.absY == 9870 || c.absX == 3146
					&& c.absY == 9871) {
	...
	public void secondClickObject(int objectType, int obX, int obY) {
		...
		case 8553:
			if (c.getItems().playerHasItem(5341)) {
				c.startAnimation(2273);
				c.sendMessage("You rake away the weeds, and it turns into a nice growing area.");
				c.getPA().object(9403, 2808, 3460, -1, 10);
			} else {
				c.sendMessage("You need a rake to eliminate the weeds in this area.");
			}
			break;
		...
		case 7848:
			if (c.getItems().playerHasItem(5341)) {
				c.startAnimation(2273);
				c.sendMessage("You rake away the weeds, and it turns into a nice growing area.");
				c.getPA().object(8049, 2809, 3463, -1, 10);
			} else {
				c.sendMessage("You need a rake to eliminate the weeds in this area.");
			}
			break;
		...
		case 11666:
		case 12100:
		case 12809:// 12806 broken, 12807 repaired, 12808 repaired with coal,
					// 12809 finished
		case 26814:
		case 21303:
			c.getSmithing().sendSmelting();
		...
		case 2646:
			...
			break;
		case 1161:
			Flax.pickCabbage(c, obX, obY);
			break;
		case 15508:
		case 15506:
		case 15507:
			Flax.pickGrain(c, obX, obY);
			break;
		/**
		 * Opening the bank.
		 */
	...
	public void thirdClickObject(int objectType, int obX, int obY) {
		...
		if (c.playerRights == 3)
			System.out.println("[" + c.playerName
					+ "]third obj click is " + objectType + " x = " + obX
					+ " y = " + obY + "\n");
		// c.sendMessage("Object type: " + objectType);
		switch (objectType) {
		case 2882:
	...
	public void firstClickNpc(int npcType) {
		if(npcType == 3) {
			DialogueManTest.enterDialogue(c, 3);
		}
		c.clickNpcType = 0;
		...
		if (c.playerRights == 3)
			System.out.println("[" + c.playerName
					+ "] first npc click is " + npcType + "\n");
		Fishing.fish(c, npcType, 1);

		switch (npcType) {
		case 37:
			c.getDH().sendDialogues(43594, 37);
			break;
		case 2824:
			TanHide.sendTanningInterface(c);
			break;
		case 744:
			c.getDH().sendNpcChat1(
					"You can cross my gangplank to entrana for 300gp", npcType,
					"Sailor");
			c.nextChat = 0;
			break;
		case 377:
			c.getDH().sendNpcChat1(
					"You can cross my gangplank to karamja for 300gp", npcType,
					"Sailor");
			c.nextChat = 0;
			break;
		case 1152:
	...
	public void secondClickNpc(int npcType) {
		...
		c.npcClickIndex = 0;
		if (c.playerRights == 3)
			System.out.println("" + c.playerName + "second obj click is "
					+ npcType + "\n");
		c.getThieving().stealFromNPC(npcType);

		Fishing.fish(c, npcType, 2);
		switch (npcType) {
		case 2824:
	...
	public void thirdClickNpc(int npcType) {
		...
		c.npcClickIndex = 0;
		if (c.playerRights == 3)
			System.out.println("npc Type third click is " + npcType);
		if (c.playerRights == 3)
			System.out.println("" + c.playerName
					+ "npc Type third click is " + npcType);
		switch (npcType) {
		case 553:
			/*
			 * for (int i = 0; i < NPCHandler.maxNPCs; i++) {
			 * 
			 * if (NPCHandler.npcs[i] != null) { if (NPCHandler.npcs[i].npcType
			 * == 553) { NPCHandler.npcs[i].updateRequired = true;
			 * NPCHandler.npcs[i].forceChat("Destineus Molenkus");
			 * c.getPA().startTeleport(2912, 4832, 0, "modern"); c.inEssence =
			 * true; } } } }
			 */
			break;
		case 542:
			c.getPA().sendFrame126("www.google.com", 12000);
			c.sendMessage("opening voting site");
			break;
		case 1599:
			c.getShops().openShop(10);
			c.sendMessage("You currently have @red@" + c.slayerPoints
