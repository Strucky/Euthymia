...
public class PlayerSave {
		...
		while (EndOfFile == false && line != null) {
			...
			if (spot > -1) {
				...
				case 2:
					...
					} else if (token.equals("arkc")) {
						p.ArmaKc = Integer.parseInt(token2);
					} else if (token.equals("zakc")) {
						p.ZammyKc = Integer.parseInt(token2);
					} else if (token.equals("bakc")) {
						p.BandKc = Integer.parseInt(token2);
					} else if (token.equals("sakc")) {
						p.SaraKc = Integer.parseInt(token2);
					} else if (token.equals("sStage")) {
						p.sStage = Integer.parseInt(token2);
				...
				//-----------------------------------------------//
					...
					} else if (token.equals("sg-points")) {
						p.sgPoints = Integer.parseInt(token2);
	...
	public static void saveGame(Client p, Runnable callback) {
		saveGame(p);
		if(callback!=null) {
			callback.run();
		}
	...
	public static boolean saveGame(Client p) {
		...
		try {
            ...
            characterfile.newLine();
			characterfile.write("arkc = ", 0, 7);
			characterfile.write(Integer.toString(p.ArmaKc), 0, Integer
					.toString(p.ArmaKc).length());
            characterfile.newLine();
			characterfile.write("bakc = ", 0, 7);
			characterfile.write(Integer.toString(p.BandKc), 0, Integer
					.toString(p.BandKc).length());
            characterfile.newLine();
			characterfile.write("zakc = ", 0, 7);
			characterfile.write(Integer.toString(p.ZammyKc), 0, Integer
					.toString(p.ZammyKc).length());
            characterfile.newLine();
			characterfile.write("sakc = ", 0, 7);
			characterfile.write(Integer.toString(p.SaraKc), 0, Integer
					.toString(p.SaraKc).length());
			characterfile.newLine();
			characterfile.write("sStage = ", 0, 9);
			characterfile.write(Integer.toString(p.sStage), 0, Integer
					.toString(p.sStage).length());
			characterfile.newLine();
			...
			characterfile.newLine(); 
			characterfile.write("sg-points = ", 0, 12);
			characterfile.write(Integer.toString(p.sgPoints), 0, Integer
					.toString(p.sgPoints).length());
			characterfile.newLine(); 
