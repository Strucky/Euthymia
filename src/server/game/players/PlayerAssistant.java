...
public class PlayerAssistant {
	...
	public void addPouches() {
		if (c.getItems().getItemCount(SMALL_POUCH) < 1
				&& c.getItems().getItemAmount(SMALL_POUCH) < 1) {
		...
		if ((c.getItems().getItemCount(MEDIUM_POUCH) < 1 && c.getItems()
				.getItemAmount(MEDIUM_POUCH) < 1)) {
			if ((c.getItems().getItemCount(BROKEN_MEDIUM_POUCH) < 1 && c
					.getItems().getItemAmount(BROKEN_MEDIUM_POUCH) < 1)) {
		...
		if ((c.getItems().getItemCount(LARGE_POUCH) < 1 && c.getItems()
				.getItemAmount(LARGE_POUCH) < 1)) {
			if ((c.getItems().getItemCount(BROKEN_LARGE_POUCH) < 1 && c
					.getItems().getItemAmount(BROKEN_LARGE_POUCH) < 1)) {
		...
		if ((c.getItems().getItemCount(GIANT_POUCH) < 1 && c.getItems()
				.getItemAmount(GIANT_POUCH) < 1)) {
			if ((c.getItems().getItemCount(BROKEN_GIANT_POUCH) < 1 && c
					.getItems().getItemAmount(BROKEN_GIANT_POUCH) < 1)) {
	...
	public void handleCommands(int gloryId) {
		c.getDH().sendOption5("Restore hp & pray.", "Boost stats.",
				"Max cash.", "QP cape please!", "QP hood please!");
		c.usingGlory = true;
		c.usingROD = false;
		// c.usingGamesNeck = false;
	}
	...
	int capes[] = { 9747, 9748, 9750, 9751, 9753, 9754, 9756, 9757, 9759, 9760,
	...
	public static int mediumLevelReward[] = { 2599, 9183, 2601, 2603, 3200,
			2605, 2607, 2609, 3100, 2611, 2613, 7334, 7340, 7346, 7352, 7358,
			7319, 7321, 7323, 7325, 7327, 7372, 7370, 7380, 7378, 2645, 2647,
			2649, 2577, 2579, 1073, 1091, 1099, 1111, 1135, 1124, 1145, 1161,
			1169, 1183, 1199, 1211, 1245, 1271, 1287, 1301, 1317, 1332, 1357,
			1371, 1430, 6916, 6918, 6920, 6922, 6924, 10400, 10402, 10416,
			11277, 10418, 10420, 10422, 10436, 10438, 10446, 10448, 10450,
	...
	public static int highLevelReward[] = { 1079, 1093, 1113, 1127, 1147, 1163,
			...
			10384, 10370, 10378, 10386, 10372, 10330, 10332, 10334, 10336,
			10338, 10340, 10342, 10344, 10346, 10348, 10350, 10352, 10380,
			10374, 10382, 10390, 10470, 10472, 10474, 10440, 10442, 10444,
			12422, 12424, 12426 };
	...
	public void makeGlobalObject(int objectId, int objectX, int objectY,
		...
		for (int i = 0; i < Config.MAX_PLAYERS; i++) {
			if (PlayerHandler.players[i] != null) {
				...
				if (c.getOutStream() != null && c != null) {
					c.getOutStream().createFrame(85);
					c.getOutStream().writeByteC(
							objectY - (c.getMapRegionY() * 8));
					c.getOutStream().writeByteC(
							objectX - (c.getMapRegionX() * 8));
					c.getOutStream().createFrame(101);
					c.getOutStream().writeByteC((objectType << 2) + (face & 3));
					c.getOutStream().writeByte(0);
	...
	public void giveLife() {
		...
		if (c.inPits) {
			...
			movePlayer(Config.FIGHT_CAVE_RESPAWN_X,
					Config.FIGHT_CAVE_RESPAWN_Y, 0);
	...
	public void followPlayer() {
		...
		if (c.isDead || c.playerLevel[3] <= 0)
			return;
		System.out.println("attempting follow");
	...
	public void followNpc() {
		...
		//c.sendMessage("otherX"+otherX);
		//c.sendMessage("otherY"+otherY);

		if(otherX == (c.absX -1) && otherY == (c.absY +1)) {
			//c.sendMessage("Disabling noclip attempt");
			return;
		}
		if(otherX == (c.absX -1) && otherY == (c.absY -1)) {
			//c.sendMessage("Disabling noclip attempt");
	...
	public void resetFollow() {
		...
		c.mageFollow = false;
		c.outStream.createFrame(174);
		// c.outStream.writeWord(2500); smith
		c.outStream.writeWord(3500);
	...
	public boolean addSkillXP(int amount, int skill) {
		...
		if (skill == 11 && amount != 1000000) {
			amount *= 20;
		}
		if (skill == 12 && amount != 1000000) {
			amount *= 20;
		}
		/*
		 * if(c.lampStart = true) { c.playerXP[skill] += (int) ((double) amount
		 * * 2); }
		 */
	...
	public void appendPoison(int damage) {
		...
		if (System.currentTimeMillis() - c.lastPoisonSip > c.poisonImmune) {
			if (c.poisonDamage > 0)
				c.sendMessage("You have been Repoisoned.");
			else
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (c.isDead || c.poisonDamage == 0 || c.poisonDamage < 0) {
						if (c.poisonDamage == 0 || c.poisonDamage < 0) {
	...
	/*public void handleLoginText() {
		...
		c.getPA().sendFrame126("Edgeville teleport", 13047);
		c.getPA().sendFrame126("Boss Teleports", 13055);
		c.getPA().sendFrame126("Draynor teleport", 13063);
		c.getPA().sendFrame126("City Teleports", 13071);
		c.getPA().sendFrame126("Minigame Teleports", 13081); // carrallanger
		c.getPA().sendFrame126("Barbarian teleport", 1300);
		c.getPA().sendFrame126("Teleports you to Barbarian village", 1301);
		c.getPA().sendFrame126("Edgeville Teleport", 1325);
		c.getPA().sendFrame126("Teleports you to Edgeville", 1326);
		c.getPA().sendFrame126("Boss Teleports", 1350);
		c.getPA().sendFrame126("Draynor teleport", 1382);
		c.getPA().sendFrame126("Teleports you to Draynor", 1383);
		c.getPA().sendFrame126("Alharid Teleport", 1415);
		c.getPA().sendFrame126("Teleports you to Alkharid", 1416);
		c.getPA().sendFrame126("Minigame Teleports", 1454); // watchtower
		c.getPA().sendFrame126("Teleports you to minigames", 1455); // watchtower
		c.getPA().sendFrame126("Catherby teleport", 7457); // trollheim
		c.getPA().sendFrame126("Teleports you to Catherby", 7458);
