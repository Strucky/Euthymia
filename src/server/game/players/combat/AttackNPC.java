...
public class AttackNPC {
	...
	public static void delayedHit(final Client c, final int i) {
		if (NPCHandler.npcs[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if(c.playerEquipment[3] == 9181 ||c.playerEquipment[3] == 9185 || c.playerEquipment[3] == 15003 || c.playerEquipment[3] == 15041) {
					if(Misc.random(10) == 1) {
						if(damage > 0) {
							c.boltDamage = damage;
		...
		if (NPCHandler.npcs[i] != null) {
			...
			if(otherX == (c.absX ) && otherY == (c.absY )) {
				...
				if (Region.getClipping((c.absX - 1), c.absY, c.heightLevel, -1, 0)) {
				 ...
				 if(c.absY == 3360||c.absY == 3359||c.absY == 3358||c.absY == 3357) {
					 if(c.absX==3113)
						 walk = 0;
				 }
				c.getPA().walkTo(walk,0);
				c.turnPlayerTo(walk,0);

			}
			/*if(NPCHandler.npcs[i].npcType == QuestHandler.WERE_WOLVE && c.playerEquipment[c.playerWeapon]!=QuestHandler.BLESSED_SICKLE){
				c.sendMessage("Hitting the werewolf damages you.");
				c.forcedChat("AAAAAAAAAAAGh");
				c.dealDamage(3);
				c.handleHitMask(3);    
			...
			if(c.attackTimer <= 0) {
				...
				c.usingCross = c.playerEquipment[c.playerWeapon] == 9185 ||c.playerEquipment[c.playerWeapon] == 9181|| c.playerEquipment[c.playerWeapon] == 15041 || c.playerEquipment[c.playerWeapon] == 15003;
				c.bonusAttack = 0;
				c.rangeItemUsed = 0;
				c.projectileStage = 0;
				if (c.autocasting) {
				...
				if (armaNpc(i) && !c.usingCross && !c.usingBow && !c.usingMagic && (c.playerEquipment[c.playerWeapon] < 4212 || c.playerEquipment[c.playerWeapon] > 4223) && !c.usingOtherRangeWeapons) {
					resetPlayerAttack(c);
					c.sendMessage("You can only attack this npc with ranged or magic!");
					return;
				}
				if (kalphite2(i) && !c.fullVerac() && !c.usingCross && !c.usingBow && !c.usingMagic && (c.playerEquipment[c.playerWeapon] < 4212 || c.playerEquipment[c.playerWeapon] > 4223) && !c.usingOtherRangeWeapons) {
					resetPlayerAttack(c);
					c.sendMessage("Your attacks have no effect on the Queen, Bring full verac, Range or Magic.");
					return;
				}
				if(c.getCombat().correctBowAndArrows() < c.playerEquipment[c.playerArrows] && Config.CORRECT_ARROWS && c.usingBow && !c.getCombat().usingCrystalBow() && c.playerEquipment[c.playerWeapon] != 9185 && c.playerEquipment[c.playerWeapon] != 9181 && c.playerEquipment[c.playerWeapon] != 15041 && c.playerEquipment[c.playerWeapon] != 15003) {
			...
			if ((c.playerEquipment[c.playerWeapon] == 9181 ||c.playerEquipment[c.playerWeapon] == 9185 || c.playerEquipment[c.playerWeapon] == 15041 || c.playerEquipment[c.playerWeapon] == 15003) && !c.getCombat().properBolts()) {
	...
	private static void resetPlayerAttack(Client c) {
		c.usingMagic = false;
		c.npcIndex = 0;
		c.faceUpdate(0);
		c.playerIndex = 0;
