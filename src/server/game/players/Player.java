...
public abstract class Player {
     ...
     *
     * @param state The state of petSummoned.
     */
    ...
    public int
            ...
            lastArrowUsed = -1, clanId = -1, autoRet = 0,
            ...
            actionTimer, rfdRound = 0, roundNpc = 0,
    ...
    public final int[] BOWS = {9181, 9185, 15003, 15041, 839, 845, 847, 851, 855,
    ...
    public boolean inWild() {
        if (absX > 2941 && absX < 3392 && absY > 3518 && absY < 3966
                ...
                || absX > 2310 && absX < 2326 && absY > 3787 && absY < 3821) {
            return true;
        }
        return false;
    }
    ...
    public boolean inMulti() {
        if (absX > 3084 && absY > 3330 && absX < 3120
                && absY < 3384 && heightLevel > 3)
            return true;
    ...
    protected void appendPlayerAppearanceOld(Stream str) {
        synchronized (this) {
    ...
    protected void appendPlayerAppearance(Stream str) {
        playerProps.currentOffset = 0;
        playerProps.writeByte(playerAppearance[0]);
        playerProps.writeString("");//playertitle added for osrs client
        playerProps.writeString("000000");//player rights color
        //rights down here
		/*PLAYER(0, "000000"),
				HELPER(11, "004080"),
				...
				OSRS(23, "437100");*/
        playerProps.writeByte(0);//playermask 	NORMAL(0), POISON(1), VENOM(2);
        playerProps.writeByte(headIcon);
        playerProps.writeByte(headIconPk);
        if (playerEquipment[playerHat] > 1) {
            playerProps.writeWord(0x200 + playerEquipment[playerHat]);
        ...
        playerProps.writeWord(playerRunIndex); // runAnimIndex
        playerProps.writeQWord(Misc.playerNameToInt64(playerName));
        playerProps.writeByte(0);//invisible ? 1 : 0
        playerProps.writeByte(combatLevel); // combat level
        playerProps.writeByte(0);//rights, list above (This is the right int)
        playerProps.writeWord(0);
        str.writeByteC(playerProps.currentOffset);
        str.writeBytes(playerProps.buffer, playerProps.currentOffset, 0);
    }
     ...
     * Face Update
     **/
    public boolean attackChanneled = false;
    public boolean playerInNewCombat = false;
