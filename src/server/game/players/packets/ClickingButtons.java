...
import server.game.players.content.skills.TanHide;
...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Client c, int packetType, int packetSize) {
		...
		if (c.tutorial == 0 && actionButtonId != 9157 && actionButtonId != 9158) {
			c.sendMessage("You must finish the tutorial");
			return;
		}
		StrongHoldOfSecurity.clickingConfirm(c, actionButtonId);

		if (c.playerRights == 3)
			c.sendMessage("[Game]" + " - actionbutton: " + actionButtonId);

		final int ADVANCED_CONFIGURATION_INTERFACE_ID =39000;
		final int ADVANCED_CONFIGURATION_BUTTON_ID=166028;
		final int ADVANCED_CONFIGURATION_CLOSE_BUTTON_ID=152109;
		switch (actionButtonId) {

			 	case ADVANCED_CONFIGURATION_CLOSE_BUTTON_ID:
				c.getPA().closeAllWindows();
				break;
			case ADVANCED_CONFIGURATION_BUTTON_ID:
				c.getPA().showInterface(ADVANCED_CONFIGURATION_INTERFACE_ID);
				break;
		...
		case 12136:
			...
			c.sendMessage("You still have " + c.taskAmount + " " + Server.npcHandler.getNpcListName(c.slayerTask) + "s "
					+ "left to kill.");
			break;
		...
		case 152255:
			c.forcedChat("I have " + c.targetPoints + " Target Points, but I've killed a total of "
		...
		case 113228:
			...
			c.sendMessage("Quests are under development.");
			break;
		case 114083:
		...
		case 113243:
			...
			c.getPA().sendFrame126("@blu@Total NPC kills: @dre@" + c.npcKills + "", 8147);
			...
			c.getPA().sendFrame126("@blu@Total Rock Crab kills: @dre@" + c.rockcrabKills + "", 8149);
			c.getPA().sendFrame126("@blu@Total Hill Giant kills: @dre@" + c.hillgiantKills + "", 8150);
			c.getPA().sendFrame126("@blu@Total Skeleton kills: @dre@" + c.skeletonKills + "", 8151);
			c.getPA().sendFrame126("@blu@Total Ghost kills: @dre@" + c.ghostKills + "", 8152);
			c.getPA().sendFrame126("@blu@Total Giant Bat kills: @dre@" + c.giantbatKills + "", 8153);
			c.getPA().sendFrame126("@blu@Total Crawling Hand kills: @dre@" + c.crawlinghandKills + "", 8154);
			c.getPA().sendFrame126("@blu@Total Chaos Dwarf kills: @dre@" + c.chaosdwarfKills + "", 8154);
			c.getPA().sendFrame126("@blu@Total Chaos Druid kills: @dre@" + c.chaosdruidKills + "", 8155);
			c.getPA().sendFrame126("@blu@Total Lesser Demon kills: @dre@" + c.lesserdemonKills + "", 8156);
			c.getPA().sendFrame126("@blu@Total Banshee kills: @dre@" + c.bansheeKills + "", 8157);
			c.getPA().sendFrame126("@blu@Total Infernal Mage kills: @dre@" + c.infernalmageKills + "", 8158);
			c.getPA().sendFrame126("@blu@Total Bloodveld kills: @dre@" + c.bloodveldKills + "", 8159);
			c.getPA().sendFrame126("@blu@Total Dust Devil kills: @dre@" + c.dustdevilKills + "", 8160);
			c.getPA().sendFrame126("@blu@Total Gargoyle kills: @dre@" + c.gargoyleKills + "", 8161);
			c.getPA().sendFrame126("@blu@Total Nechryael kills: @dre@" + c.nechryaelKills + "", 8162);
			c.getPA().sendFrame126("@blu@Total Black Demon kills: @dre@" + c.blackdemonKills + "", 8163);
			c.getPA().sendFrame126("@blu@Total Green Dragon kills: @dre@" + c.greendragonKills + "", 8164);
			c.getPA().sendFrame126("@blu@Total Blue Dragon kills: @dre@" + c.bluedragonKills + "", 8165);
			c.getPA().sendFrame126("@blu@Total Baby Blue Dragon  kills: @dre@" + c.babybluedragonKills + "", 8166);
			c.getPA().sendFrame126("@blu@Total Bronze Dragon kills: @dre@" + c.bronzedragonKills + "", 8167);
			c.getPA().sendFrame126("@blu@Total Iron Dragon kills: @dre@" + c.irondragonKills + "", 8168);
			c.getPA().sendFrame126("@blu@Total Steel Dragon kills: @dre@" + c.steeldragonKills + "", 8169);
			c.getPA().sendFrame126("@blu@Total Dark Beast kills: @dre@" + c.darkBeastKills + "", 8170);
			c.getPA().sendFrame126("@blu@Total Abyssal Demon kills: @dre@" + c.abbyDemonKills + "", 8171);
		...
		case 113241:
			...
			c.getPA().sendFrame126("@dre@Achievements: You currently have " + c.aPoints + " points.", 8144);
			c.getPA().sendFrame126("", 8145);
			if (c.KC > 0 && c.KC < 25) {
			...
			if (c.logsCut >= 1000) {
				...
				c.getPA().sendFrame126("@yel@" + c.logsCut + "/1000 Logs fletched", 8155);
			} else {
			...
			if (c.logsCut >= 2500) {
				...
				c.getPA().sendFrame126("@yel@" + c.logsCut + "/2500 Logs fletched", 8156);
			} else {
			...
			if (c.logsCut >= 5000) {
				...
				c.getPA().sendFrame126("@yel@" + c.logsCut + "/5000 Logs fletched", 8157);
			} else {
			...
			if (c.amountFished >= 500) {
				...
				c.getPA().sendFrame126("@yel@" + c.amountFished + "/500 Fishes caught", 8158);
			} else {
			...
			if (c.amountFished >= 1000) {
				...
				c.getPA().sendFrame126("@yel@" + c.amountFished + "/1000 Fishes caught", 8159);
			} else {
			...
			if (c.amountFished >= 1700) {
				...
				c.getPA().sendFrame126("@yel@" + c.amountFished + "/1700 Fishes caught", 8160);
			} else {
			...
			if (c.amountFished >= 2500) {
				...
				c.getPA().sendFrame126("@yel@" + c.amountFished + "/2500 Fishes caught", 8161);
			} else {
			...
			if (c.amountFished >= 5000) {
				...
				c.getPA().sendFrame126("@yel@" + c.amountFished + "/5000 Fishes caught", 8162);
			} else {
			...
			if (c.oresMined >= 1000) {
				...
				c.getPA().sendFrame126("@yel@" + c.oresMined + "/1000 Ores mined", 8163);
			} else {
			...
			if (c.oresMined >= 249) {
				...
				c.getPA().sendFrame126("@yel@" + c.oresMined + "/250 Ores mined", 8164);
			} else {
			...
			if (c.oresMined >= 500) {
				...
				c.getPA().sendFrame126("@yel@" + c.oresMined + "/500 Ores mined", 8165);
			} else {
			...
			if (c.oresMined >= 2500) {
				...
				c.getPA().sendFrame126("@yel@" + c.oresMined + "/2500 Ores mined", 8166);
			} else {
			...
			if (c.oresMined >= 5000) {
				...
				c.getPA().sendFrame126("@yel@" + c.oresMined + "/5000 Ores mined", 8167);
			} else {
			...
			if (c.woodChopped >= 1000) {
				...
				c.getPA().sendFrame126("@yel@" + c.woodChopped + "/1000 Wood chopped", 8168);
			} else {
			...
			if (c.woodChopped >= 2500) {
				...
				c.getPA().sendFrame126("@yel@" + c.woodChopped + "/2500 Wood chopped", 8169);
			} else {
			...
			if (c.woodChopped >= 5000) {
				...
				c.getPA().sendFrame126("@yel@" + c.woodChopped + "/5000 Wood chopped", 8170);
			} else {
			...
			if (c.fishCooked >= 1000) {
				...
				c.getPA().sendFrame126("@yel@" + c.fishCooked + "/1000 Fishes cooked", 8171);
			} else {
			...
			if (c.fishCooked >= 2500) {
				...
				c.getPA().sendFrame126("@yel@" + c.fishCooked + "/2500 Fishes cooked", 8172);
			} else {
			...
			if (c.fishCooked >= 5000) {
				...
				c.getPA().sendFrame126("@yel@" + c.fishCooked + "/5000 Fishes cooked", 8173);
			} else {
			...
			if (c.bossKills >= 50) {
				...
				c.getPA().sendFrame126("@yel@" + c.bossKills + "/50 Boss Kills", 8174);
			} else {
			...
			if (c.bossKills >= 175) {
				...
				c.getPA().sendFrame126("@yel@" + c.bossKills + "/175 Boss Kills", 8175);
			} else {
			...
			if (c.bossKills >= 250) {
				...
				c.getPA().sendFrame126("@yel@" + c.bossKills + "/250 Boss Kills", 8176);
			} else {
			...
			if (c.bossKills >= 500) {
				...
				c.getPA().sendFrame126("@yel@" + c.bossKills + "/500 Boss Kills", 8178);
			} else {
			...
			if (c.bossKills >= 1000) {
				...
				c.getPA().sendFrame126("@yel@" + c.bossKills + "/1000 Boss Kills", 8179);
			} else {
			...
			if (c.bossKills >= 2000) {
				...
				c.getPA().sendFrame126("@yel@" + c.bossKills + "/2000 Boss Kills", 8180);
			} else {
		...
		case 113242:
			...
			// c.getPA().sendFrame126("@gre@Total Boss kills: " + c.bossKills +
			// "", 8148);
			c.getPA().sendFrame126("@blu@Armadyl kills: @dre@" + c.armadylKills + "", 8147);
			c.getPA().sendFrame126("@blu@Bandos kills: @dre@" + c.bandosKills + "", 8148);
			c.getPA().sendFrame126("@blu@Saradomin kills: @dre@" + c.saradominKills + "", 8149);
			c.getPA().sendFrame126("@blu@Zamorak kills: @dre@" + c.zamorakKills + "", 8150);
			c.getPA().sendFrame126("@blu@Dagannoth Supreme kills: @dre@" + c.dSupremeKills + "", 8151);
			c.getPA().sendFrame126("@blu@Dagannoth Prime kills:@dre@ " + c.dPrimeKills + "", 8152);
			c.getPA().sendFrame126("@blu@Dagannoth Rex kills: @dre@" + c.dRexKills + "", 8153);
			c.getPA().sendFrame126("@blu@Scorpia kills: @dre@" + c.scorpiaKills + "", 8154);
			c.getPA().sendFrame126("@blu@Vet'ion kills: @dre@" + c.vetionKills + "", 8155);
			c.getPA().sendFrame126("@blu@Venenatis kills: @dre@" + c.venenatisKills + "", 8156);
			c.getPA().sendFrame126("@blu@Callisto kills: @dre@" + c.callistoKills + "", 8157);
			c.getPA().sendFrame126("@blu@Chaos Elemental kills: @dre@" + c.chaosKills + "", 8158);
			c.getPA().sendFrame126("@blu@Sea Troll Queen kills: @dre@" + c.seaTrollQueenKills + "", 8159);
			c.getPA().sendFrame126("@blu@Giant Sea Snake kills: @dre@" + c.seaSnakeKills + "", 8160);
			c.getPA().sendFrame126("@blu@Penance Queen kills: @dre@" + c.penanceQueenKills + "", 8161);
			c.getPA().showInterface(8134);
			break;

		/*
		 * case 113246: // c.getDH().sendDialogues(4321, 113244); for (int i = 8144; i <
		 * 8200; i++) { c.getPA().sendFrame126("", i); }
		 * c.getPA().sendFrame126("@dre@                 Command List", 8144);
		 * c.getPA().sendFrame126("::shops", 8145);
		 * c.getPA().sendFrame126("::experience", 8146);
		 * c.getPA().sendFrame126("::task", 8147); c.getPA().sendFrame126("::resettask",
		 * 8148); c.getPA().sendFrame126("::maxhit", 8149);
		 * c.getPA().sendFrame126("::skull", 8150); c.getPA().sendFrame126("::forum",
		 * 8151); c.getPA().sendFrame126("::updates", 8152);
		 * c.getPA().sendFrame126("::donate", 8153); c.getPA().sendFrame126("::vote",
		 * 8154); c.getPA().sendFrame126("::home", 8155);
		 * c.getPA().sendFrame126("::train", 8156); c.getPA().sendFrame126("::agility",
		 * 8150); c.getPA().sendFrame126("::ardy", 8150);
		 * c.getPA().sendFrame126("::barrows", 8150); c.getPA().sendFrame126("::tzhaar",
		 * 8150);
		 * 
		...
		case 15159:
			if (c.smeltInterface) {
				c.getPA().closeAllWindows();
				if (c.getItems().playerHasItem(453, 2) && c.getItems().playerHasItem(440, 1)
						&& (c.playerLevel[13] >= 30)) {
		...
		case 15158:
			if (c.smeltInterface) {
				c.getPA().closeAllWindows();
				if (c.getItems().playerHasItem(453, 10) && c.getItems().playerHasItem(440, 5)
						&& (c.playerLevel[13] >= 30)) {
		...
		case 15157:
			if (c.smeltInterface) {
				c.getPA().closeAllWindows();
				if (c.getItems().playerHasItem(453, 20) && c.getItems().playerHasItem(440, 8)
						&& (c.playerLevel[13] >= 30)) {
					...
					c.sendMessage(
							"You need to have atleast 30 smithing 8 iron ore and 20 coal to smelt 10 steel bars.");
					c.getPA().removeAllWindows();
					c.smeltInterface = false;
				}
			} else {
		...
		case 29017:
			if (c.smeltInterface) {
				c.getPA().closeAllWindows();
				if (c.getItems().playerHasItem(447, 1) && c.getItems().playerHasItem(453, 2)
						&& (c.playerLevel[13] >= 50)) {
					...
					c.getPA().addSkillXP(30 * Config.SMITHING_EXPERIENCE,
							c.playerSmithing);
					c.getPA().refreshSkill(c.playerSmithing);
				} else {
		...
		case 29016:
			if (c.smeltInterface) {
				c.getPA().closeAllWindows();
				if (c.getItems().playerHasItem(447, 5) && c.getItems().playerHasItem(453, 10)
						&& (c.playerLevel[13] >= 50)) {
					...
					c.getPA().addSkillXP(30 * 5 * Config.SMITHING_EXPERIENCE,
							c.playerSmithing);
					c.getPA().refreshSkill(c.playerSmithing);
				} else {
		...
		case 24253:
			if (c.smeltInterface) {
				c.getPA().closeAllWindows();
				if (c.getItems().playerHasItem(447, 9) && c.getItems().playerHasItem(453, 18)
						&& (c.playerLevel[13] >= 50)) {
					c.sendMessage("You begin to smelt 9 mithril bars..");
					c.getItems().deleteItem1(447, 9);
					c.getItems().deleteItem1(453, 18);
					c.startAnimation(899);
					c.getItems().addItem(2359, 9);
					c.getPA().addSkillXP(30 * 9 * Config.SMITHING_EXPERIENCE,
							c.playerSmithing);
					c.getPA().refreshSkill(c.playerSmithing);
				} else {
		...
		case 29022:
			if (c.smeltInterface) {
				c.getPA().closeAllWindows();
				if (c.getItems().playerHasItem(449, 1) && c.getItems().playerHasItem(453, 4)
						&& (c.playerLevel[13] >= 70)) {
					c.sendMessage("You begin to smelt an adamant bar..");
					c.getItems().deleteItem1(449, 1);
					c.getItems().deleteItem1(453, 4);
					c.startAnimation(899);
					c.getItems().addItem(2361, 1);
					c.getPA().addSkillXP(38 * Config.SMITHING_EXPERIENCE,
							c.playerSmithing);
					c.getPA().refreshSkill(c.playerSmithing);
				} else {
		...
		case 29020:
			if (c.smeltInterface) {
				c.getPA().closeAllWindows();
				if (c.getItems().playerHasItem(449, 5) && c.getItems().playerHasItem(453, 20)
						&& (c.playerLevel[13] >= 70)) {
					...
					c.getPA().addSkillXP(38 * 5 * Config.SMITHING_EXPERIENCE,
							c.playerSmithing);
					c.getPA().refreshSkill(c.playerSmithing);
				} else {
		...
		case 29026:
			if (c.smeltInterface) {
				c.getPA().closeAllWindows();
				if (c.getItems().playerHasItem(451, 1) && c.getItems().playerHasItem(453, 6)
						&& (c.playerLevel[13] >= 85)) {
					c.sendMessage("You begin to smelt a rune bar..");
					c.getItems().deleteItem1(451, 1);
					c.getItems().deleteItem1(453, 6);
					c.startAnimation(899);
					c.getItems().addItem(2363, 1);
					c.getPA().addSkillXP(50 * Config.SMITHING_EXPERIENCE,
							c.playerSmithing);
					c.getPA().refreshSkill(c.playerSmithing);
				} else {
		...
		case 29025:
			if (c.smeltInterface) {
				c.getPA().closeAllWindows();
				if (c.getItems().playerHasItem(451, 4) && c.getItems().playerHasItem(453, 24)
						&& (c.playerLevel[13] >= 85)) {
					c.sendMessage("You begin to smelt 4 rune bars..");
					c.getItems().deleteItem1(451, 4);
					c.getItems().deleteItem1(453, 24);
					c.startAnimation(899);
					c.getItems().addItem(2363, 4);
					c.getPA().addSkillXP(50 * 4 * Config.SMITHING_EXPERIENCE,
							c.playerSmithing);
					c.getPA().refreshSkill(c.playerSmithing);
				} else {
		 ...
		 * c.getSI().attackComplex(1); c.getSI().selected = 0; } break; case 33209:
		 * if(c.isTraining == false){ c.outStream.createFrame(27); c.strengthSkill =
		 * true; c.attackSkill = false; c.mageSkill = false; c.rangeSkill = false;
		 * c.defenceSkill = false; c.prayerSkill = false; c.healthSkill = false; } else
		 * if(c.isTraining == true){ c.getSI().strengthComplex(1); c.getSI().selected =
		 * 1; } break; case 33212: if(c.isTraining == false){
		 * c.outStream.createFrame(27); c.defenceSkill = true; c.attackSkill = false;
		 * c.strengthSkill = false; c.mageSkill = false; c.rangeSkill = false;
		 * c.prayerSkill = false; c.healthSkill = false; } else if(c.isTraining ==
		 * true){ c.getSI().defenceComplex(1); c.getSI().selected = 2; } break; case
		 * 33215: if(c.isTraining == false){ c.outStream.createFrame(27); c.rangeSkill =
		 * true; c.attackSkill = false; c.strengthSkill = false; c.mageSkill = false;
		 * c.defenceSkill = false; c.prayerSkill = false; c.healthSkill = false; } else
		 * if(c.isTraining == true){ c.getSI().rangedComplex(1); c.getSI().selected = 3;
		 * } break; case 33218: if(c.isTraining == false){ c.outStream.createFrame(27);
		 * c.prayerSkill = true; c.attackSkill = false; c.strengthSkill = false;
		 * c.mageSkill = false; c.rangeSkill = false; c.defenceSkill = false;
		 * c.healthSkill = false; } else if(c.isTraining == true){
		 * c.getSI().prayerComplex(1); c.getSI().selected = 4; } break; case 33221:
		 * if(c.isTraining == false){ c.outStream.createFrame(27); c.mageSkill = true;
		 * c.attackSkill = false; c.strengthSkill = false; c.rangeSkill = false;
		 * c.defenceSkill = false; c.prayerSkill = false; c.healthSkill = false; } else
		 * if(c.isTraining == true){ c.getSI().magicComplex(1); c.getSI().selected = 5;
		 * } break; case 33207: if(c.isTraining == false){ c.outStream.createFrame(27);
		...
		/*
		 * case 33206: // attack c.getSI().attackComplex(1); c.getSI().selected = 0;
		 * break; case 33209: // strength c.getSI().strengthComplex(1);
		 * c.getSI().selected = 1; break; case 33212: // Defence
		 * c.getSI().defenceComplex(1); c.getSI().selected = 2; break; case 33215: //
		 * range c.getSI().rangedComplex(1); c.getSI().selected = 3; break; case 33218:
		 * // prayer c.getSI().prayerComplex(1); c.getSI().selected = 4; break; case
		 * 33221: // mage c.getSI().magicComplex(1); c.getSI().selected = 5; break;
		 */
		case 33224: // runecrafting
			...
			break;
		/*
		 * case 33207: // hp c.getSI().hitpointsComplex(1); c.getSI().selected = 7;
		 * break;
		 */
		case 33208:
		...
		case 9190:
			...
			if (c.dialogueAction == 5555) {
				if (c.attackLevelReq == 99 && c.defenceLevelReq == 99) {
					...
					c.titleColor = 3;
					c.sendMessage("You succesfully changed your title to @blu@" + c.playerTitle + ".");
					c.dialogueAction = -1;
					c.getPA().removeAllWindows();
				} else {
		...
		case 9191:
			if (c.teleAction == 14) {
				if (c.playerLevel[c.playerFishing] < 70) {
					c.dialogueAction = -1;
					c.sendMessage("You need a fishing level of 70 to enter the guild.");
					c.getPA().closeAllWindows();
					break;
			...
			/*
			 * if (c.teleAction == 1) { //rock crabs c.getPA().spellTeleport(2676, 3715, 0);
			 * } else if (c.teleAction == 2) { //taverly dungeon
			 * c.getPA().spellTeleport(2884, 9798, 0); } else if (c.teleAction == 3) { //kbd
			 * c.getPA().spellTeleport(3007, 3849, 0); } else if (c.teleAction == 4) {
			 * //west lv 10 wild c.getPA().spellTeleport(2979, 3597, 0); } else if
			 * (c.teleAction == 5) { c.getPA().spellTeleport(3079,9502,0); }
			 */
			if (c.teleAction == 1) {
				// slay dungeon
		...
		case 9192:
			...
			/*
			 * if (c.teleAction == 1) { //experiments c.getPA().spellTeleport(3555, 9947,
			 * 0); } else if (c.teleAction == 2) { //brimhavem dung
			 * c.getPA().spellTeleport(2709, 9564, 0); } else if (c.teleAction == 3) { //dag
			 * kings c.getPA().spellTeleport(2479, 10147, 0); } else if (c.teleAction == 4)
			 * { //easts lv 18 c.getPA().spellTeleport(3351, 3659, 0); } else if
			 * (c.teleAction == 5) { c.getPA().spellTeleport(2813,3436,0); }
			 */
			if (c.teleAction == 1) {
				// slayer tower
		...
		case 9193:
			...
			if (c.teleAction == 20) {
				c.getPA().spellTeleport(3038, 3352, 0);
				break;
			}
		...
		case 9194:
			...
			/*
			 * if (c.teleAction == 21) { //yanille c.getPA().spellTeleport(3361, 9550, 0); }
			 */
			if (c.teleAction == 10) {
			...
			/*
			 * if (c.teleAction == 1) { //island c.getPA().spellTeleport(2895, 2727, 0); }
			 * else if (c.teleAction == 2) { //last minigame spot
			 * c.sendMessage("Suggest something for this spot on the forums!");
			 * c.getPA().closeAllWindows(); } else if (c.teleAction == 3) { //last monster
			 * spot c.sendMessage("Suggest something for this spot on the forums!");
			 * c.getPA().closeAllWindows(); } else if (c.teleAction == 4) { //dark castle
			 * multi easts c.getPA().spellTeleport(3037, 3652, 0); } else if (c.teleAction
			 * == 5) { c.getPA().spellTeleport(2812,3463,0); }
			 */

			if (c.teleAction == 1) {
				// traverly
				// c.getPA().spellTeleport(3297, 9824, 0);
				// c.sendMessage("@red@There's just frost dragons, if you want to kill green
				// dragons you must go wilderness.");
		...
		case 34190:
			...
			break;
	/*	case 74212:// fullscreen
			switch (c.screenCount) {
			case 0:
				c.forcedChat("::resize");
				c.screenCount=1;
				break;
			case 1:
				c.forcedChat(":fullscreen:");
				c.screenCount=2;
				break;
			case 2:
				c.screenCount=0;
				c.forcedChat("::regular");
				break;
			}
			break;*/
		// case 58253:
		...
		case 9178:
			...
			if (c.dialogueAction == 1658) {
				...
				c.getItems().deleteItem(995, c.getItems().getItemSlot(995), 17000000);
		...
		case 9180:
			...
			if (c.teleAction == 21) {
				// duel arena
				c.getPA().spellTeleport(3366, 3266, 0);
				// c.sendMessage("Dueling/Staking is a bit broken at the moment. Recommending to
				// use Middleman!");
			}
			...
			if (c.dialogueAction == 5) {
				 ...
				 * Server.npcHandler.getNpcListName(c.slayerTask) + " ", 7383);
				 */
				c.getPA().closeAllWindows();
			}
			if (c.usingROD) {
		...
		case 1097:
			if (c.autocastId > 0) {
				...
				if (c.playerMagicBook == 1) {
					if (c.playerEquipment[c.playerWeapon] == 4675 || c.playerEquipment[c.playerWeapon] == 15110
							|| c.playerEquipment[c.playerWeapon] == 11994)
		...
		case 9157:
			if(c.getDH().isInDialogue()) {
				c.getDH().getCurrentDialogue().handleFirstOption.run();
			}


			if (c.dialogueAction == 43595) {
			...
			if (c.sStage == 4) {
				...
				c.getDH().sendItemChat1("Choosing boots", "Give me the colorful ones", 9005, 350);
				c.sStage = 3;
				break;
			}
			if (c.teleAction == 21) {
			...
			if (c.dialogueAction == 1002) {
				...
				if (c.getItems().freeSlots() > 0) {
					...
					c.sendAll("@red@" + c.playerName + " opened barrows chest and received a "
			...
			if (c.dialogueAction == 2258) {
				...
				if (c.dp >= 10) {
					if (c.getItems().freeSlots() > 2) {
						...
						c.sendAll("@red@" + c.playerName + " opened the Magic Chest and received a "
								+ c.getItems().getItemName(itemGat) + "!");
		...
		case 9167:
			...
			/*
			 * if (c.dialogueAction == 4321) { c.getPA().showInterface(8134); for(int i =
			 * 8144; i < 8195; i++) { c.getPA().sendFrame126("", i); }
			 * c.getPA().sendFrame126("@dre@PvP Achievements", 8144);
			 * c.getPA().sendFrame126("", 8145); if (c.KC > 0 && c.KC < 25) {
			 * c.getPA().sendFrame126("@yel@"+ c.KC +"/25 Kills", 8147); } else if (c.KC >=
			 * 25) { c.getPA().sendFrame126("@gre@25 Kills", 8147); } else {
			 * c.getPA().sendFrame126("@red@20 Kills", 8147); } if (c.KC > 25 && c.KC < 50)
			 * { c.getPA().sendFrame126("@yel@"+ c.KC +"/50 Kills", 8148); } else if (c.KC
			 * >= 50) { c.getPA().sendFrame126("@gre@50 Kills", 8148); } else {
			 * c.getPA().sendFrame126("@red@50 Kills", 8148); } if (c.KC > 50 && c.KC < 150)
			 * { c.getPA().sendFrame126("@yel@"+ c.KC +"/150 Kills", 8149); } else if (c.KC
			 * >= 150) { c.getPA().sendFrame126("@gre@150 Kills", 8149); } else {
			 * c.getPA().sendFrame126("@red@150 Kills", 8149); } if (c.KC > 150 && c.KC <
			 * 300) { c.getPA().sendFrame126("@yel@"+ c.KC +"/300 Kills", 8150); } else if
			 * (c.KC >= 300) { c.getPA().sendFrame126("@gre@300 Kills", 8150); } else {
			 * c.getPA().sendFrame126("@red@300 Kills", 8150); } if (c.KC > 300 && c.KC <
			 * 500) { c.getPA().sendFrame126("@yel@"+ c.KC +"/500 Kills", 8151); } else if
			 * (c.KC >= 500) { c.getPA().sendFrame126("@gre@500 Kills", 8151); } else {
			 * c.getPA().sendFrame126("@red@500 Kills", 8151); } if (c.KC > 500 && c.KC <
			 * 1000) { c.getPA().sendFrame126("@yel@"+ c.KC +"/1000 Kills", 8151); } else if
			 * (c.KC >= 1000) { c.getPA().sendFrame126("@gre@1000 Kills", 8151); } else {
			 * c.getPA().sendFrame126("@red@1000 Kills", 8151); } }
			 */
		...
		case 9168:
			/*
			 * if (c.dialogueAction == 209) { // c.newPlayerAct = 1; // c.trade11 = 900; //
			 * time till they can trade c.isTraining = true; c.chosenstarter = true;
			 * c.playerTitle = ""; c.titleColor = 3; c.getPA().removeAllWindows(); //
			 * c.isNewPlayer = true; if (!Connection
			 ...
			 * Server.playerHandler.players[j];
			 * c2.sendMessage("[@cr8@@mag@New Player@bla@] @blu@" + c.playerName +
			 * " @bla@has logged in! Welcome!"); } }
			 * 
			 * c.getItems().addItem(995, 250000); c.getItems().addItem(380, 100);
			 * c.getItems().addItem(1323, 1); c.getItems().addItem(1333, 1);
			 * c.getItems().addItem(1153, 1); c.getItems().addItem(1115, 1);
			 * c.getItems().addItem(1067, 1); c.getItems().addItem(3842, 1);
			 * c.getItems().addItem(1725, 1); c.getItems().addItem(3105, 1);
			 * c.getItems().addItem(4367, 1); c.getItems().addItem(544, 1);
			 * c.getItems().addItem(1129, 1); c.getItems().addItem(1059, 1);
			 * c.getItems().addItem(1095, 1); c.getItems().addItem(542, 1);
			 * c.getItems().addItem(556, 100); c.getItems().addItem(554, 100);
			 * c.getItems().addItem(558, 100); c.getItems().addItem(557, 100);
			 * c.getItems().addItem(555, 100); c.getItems().addItem(841, 1);
			 * c.getItems().addItem(884, 50); c.setSidebarInterface(6, 1151); // pure
			 * c.getPA().removeAllWindows(); Connection .addIpToStarterList1
			 * (PlayerHandler.players[c.playerId].connectedFrom); Connection
			 * .addIpToStarter1 (PlayerHandler.players[c.playerId].connectedFrom);
			 * c.sendMessage(
			 * "You have recieved 1 out of 2 starter packages on this IP address." ); } else
			 * if (Connection .hasRecieved1stStarter(PlayerHandler.players
			 * [c.playerId].connectedFrom) && !Connection .hasRecieved2ndStarter(
			 * PlayerHandler.players[c.playerId].connectedFrom)) { c.getItems().addItem(995,
			 * 150000); c.getItems().addItem(380, 100); c.getItems().addItem(1323, 1);
			 * c.getItems().addItem(1333, 1); c.getItems().addItem(1153, 1);
			 * c.getItems().addItem(1115, 1); c.getItems().addItem(1067, 1);
			 * c.getItems().addItem(3842, 1); c.getItems().addItem(1725, 1);
			 * c.getItems().addItem(3105, 1); c.getItems().addItem(4367, 1);
			 * c.getItems().addItem(544, 1); c.getItems().addItem(1129, 1);
			 * c.getItems().addItem(1059, 1); c.getItems().addItem(1095, 1);
			 * c.getItems().addItem(542, 1); c.getItems().addItem(556, 100);
			 * c.getItems().addItem(554, 100); c.getItems().addItem(558, 100);
			 * c.getItems().addItem(557, 100); c.getItems().addItem(555, 100);
			 * c.getItems().addItem(841, 1); c.getItems().addItem(884, 50);
			 * c.setSidebarInterface(6, 1151); // pure c.getPA().removeAllWindows();
			 * c.sendMessage(
			 * "You have recieved 2 out of 2 starter packages on this IP address." );
			 * Connection .addIpToStarterList2(PlayerHandler.players[c.playerId
			 * ].connectedFrom); Connection .addIpToStarter2(PlayerHandler.players
			 * [c.playerId].connectedFrom); } else if (Connection .hasRecieved1stStarter
			 * (PlayerHandler.players[c.playerId].connectedFrom) && Connection
		...
		case 9169:
			/*
			 * if (c.dialogueAction == 209) { // .getDH().sendDialogues(209, 0); //
			 * c.newPlayerAct = 1; // c.trade11 = 900; // time till they can trade
			 * c.isTraining = true; c.chosenstarter = true; c.playerTitle = ""; c.titleColor
			 * = 3; c.getPA().removeAllWindows(); // c.isNewPlayer = true; if (!Connection
			 * .hasRecieved1stStarter(PlayerHandler .players[c.playerId].connectedFrom)) {
			 * for (int j = 0; j < Server.playerHandler.players.length; j++) { if
			 * (Server.playerHandler.players[j] != null) { Client c2 = (Client)
			 * Server.playerHandler.players[j];
			 ...
			 * " @bla@has logged in! Welcome!"); } }
			 * 
			 * c.getItems().addItem(995, 200000); // skill package c.getItems().addItem(228,
			 * 100); c.getItems().addItem(1437, 250); c.getItems().addItem(314, 1000);
			 * c.getItems().addItem(1267, 1); c.getItems().addItem(1271, 1);
			 * c.getItems().addItem(1349, 1); c.getItems().addItem(1357, 1);
			 * c.getItems().addItem(303, 1); c.getItems().addItem(305, 1);
			 * c.getItems().addItem(301, 1); c.getItems().addItem(311, 1);
			 * c.getItems().addItem(2347, 1); c.getItems().addItem(946, 1);
			 * c.getItems().addItem(1755, 1); c.getItems().addItem(590, 1);
			 * c.getItems().addItem(952, 1); c.getItems().addItem(10400, 1);
			 * c.getItems().addItem(10402, 1); c.getItems().addItem(1949, 1);
			 * c.playerMagicBook = 0; c.getPA().removeAllWindows(); Connection
			 * .addIpToStarterList1(PlayerHandler .players[c.playerId].connectedFrom);
			 * Connection .addIpToStarter1(PlayerHandler
			 * .players[c.playerId].connectedFrom); c.sendMessage(
			 * "You have recieved 1 out of 2 starter packages on this IP address." ); } else
			 * if (Connection .hasRecieved1stStarter(PlayerHandler.players
			 * [c.playerId].connectedFrom) && !Connection .hasRecieved2ndStarter(
			 * PlayerHandler.players[c.playerId].connectedFrom)) { c.getItems().addItem(995,
			 * 100000); // skill package c.getItems().addItem(228, 100);
			 * c.getItems().addItem(1437, 1000); c.getItems().addItem(314, 500);
			 * c.getItems().addItem(1267, 1); c.getItems().addItem(1271, 1);
			 * c.getItems().addItem(1349, 1); c.getItems().addItem(1357, 1);
			 * c.getItems().addItem(303, 1); c.getItems().addItem(305, 1);
			 * c.getItems().addItem(301, 1); c.getItems().addItem(311, 1);
			 ...
			 * c.getItems().addItem(952, 1); c.getItems().addItem(10400, 1);
			 * c.getItems().addItem(10402, 1); c.getItems().addItem(1949, 1);
			 * c.playerMagicBook = 0; c.getPA().removeAllWindows(); c.sendMessage(
			 * "You have recieved 2 out of 2 starter packages on this IP address." );
			 * Connection .addIpToStarterList2(PlayerHandler.players[c.playerId
			 * ].connectedFrom); Connection .addIpToStarter2(PlayerHandler.players
			 * [c.playerId].connectedFrom); } else if (Connection .hasRecieved1stStarter
		...
		case 9158:
			if(c.getDH().isInDialogue()) {
				c.getDH().getCurrentDialogue().handleSecondOption.run();
			}

			if (c.dialogueAction == 300115) {
				c.getPA().closeAllWindows();
				c.nextChat = 0;
				c.dialogueAction = -1;
			...
			if (c.dialogueAction == 300084) {
				c.getDH().sendPlayerChat1("Good luck finding your hero");
				c.dialogueAction = -1;
				c.nextChat = 0;
				break;
			}

			if (c.dialogueAction == 300040) {
				c.dialogueAction = -1;
				c.getPA().closeAllWindows();
				break;
			}
			...
			if (c.tutorial == 0) {
				c.getDH().sendDialogues(300012, 1);
				break;
			}
			if (c.sStage == 4) {
				c.getItems().addItem(9006, 1);
				c.getDH().sendItemChat1("Choosing boots", "Give me the fighting looking pair", 9006, 350);
			...
			if (c.dialogueAction == 209) {
				...
				for (int z = 0; z < PlayerHandler.players.length; z++) {
					if (PlayerHandler.players[z] != null) {
						...
						c2.sendMessage("@red@[News:]@bla@ Welcome @blu@" + c.playerName
		...
		case 117112:
			...
			// "modern");
			c.getPA().startTeleport(3084, 3424, 0, "modern");

			break;

		case 4143:
			c.getPA().startTeleport(3087, 3503, 0, "modern");

			break;
		case 29031:
			c.getPA().startTeleport(2820, 3460, 0, "modern");

			// c.getDH().sendOption5("Woodcutting", "Fishing",
			// "Mining & Smithing","Agility", "Farming");
			// c.teleAction = 14;
			break;
		/*
		 * case 50245: case 117123: // c.getDH().sendOption5("Barrows", "", "",
		 * "Duel Arena", ""); // c.teleAction = 2; c.getPA().startTeleport(2149, 5103,
		 * 0, "modern"); break;
		 */
		...
		case 4150:
			 ...
			 * "Lesser Demons (level 46)", "Chaos Temple"); c.teleAction = 4;
			 */
		...
		case 117194:
			/*
			 * c.getDH().sendOption5("Lumbridge", "Varrock", "Catherby", "Falador",
			 * "Yanille"); c.teleAction = 20;
			 */
			c.getPA().startTeleport(3301, 3202, 0, "modern");
			break;

		case 51023:
		case 6005:
			c.getDH().sendOption4("Pest Control", "Barrows", "Wave Survival", "Fight Caves");
			c.teleAction = 21;
			break;
		case 117162:
			/*
