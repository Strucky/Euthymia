package server.game.players.packets;

import server.Server;
import server.clip.ClippedTiles;
import server.event.Task;
import server.game.players.Client;
import server.game.players.InterfaceEvent;
import server.game.players.PacketType;
import server.game.players.PlayerHandler;
import server.game.players.StrongHoldOfSecurity;
import server.game.players.action.Action;
import server.game.players.content.skills.SkillHandler;
import server.Config;
import server.game.items.CombinedItems;
import server.game.items.GameItem;
import server.game.items.ItemAssistant;
import server.util.Misc;
import server.game.players.content.Teles;

/**
 * Walking packet
 **/
public class Walking implements PacketType {



	@Override
	public void processPacket(Client c, int packetType, int packetSize) {
;
		if (c.stopPlayerSkill) {
			c.getPA().resetPlayerSkillVariables(c);
			c.stopPlayerSkill = false;
		}

		if(c.canChangeAppearance)
			c.canChangeAppearance = false;
		if(c.objectAction5 > 0)
			StrongHoldOfSecurity.setObjectUsingFalse(c, 3);

		if(c.tutorial == 0) {
			c.sendMessage("You must complete the tutorial");
			return;
		}
		if (c.sStage == 4)
			c.sStage = 3;
		if (c.isFishingTrune) {
			c.startAnimation(65535);
			Server.getTaskScheduler().schedule(new Task(1, false) {

				@Override
				protected void execute() {
					c.startAnimation(65535);
					c.isFishingTrune = false;

					stop();
				}
			});
		}
	//	if(c.dialogueAction > 0)
		//		c.dialogueAction = 0;
		if (c.isBanking)
			c.isBanking = false;
		// if(c.pickingLaundry)
		// c.pickingLaundry = false;
		if (packetType == 248 || packetType == 164) {
			c.faceUpdate(0);
			c.npcIndex = 0;
			c.playerIndex = 0;
			if (c.followId > 0 || c.followId2 > 0 || c.follow2 > 0)
				c.getPA().resetFollow();
		}
		c.getPA().removeAllWindows();
		if (c.duelRule[1] && c.duelStatus == 5) {
			if (Server.playerHandler.players[c.duelingWith] != null) {
				if (!c.goodDistance(c.getX(), c.getY(),
						Server.playerHandler.players[c.duelingWith].getX(),
						Server.playerHandler.players[c.duelingWith].getY(), 1)
						|| c.attackTimer == 0) {
					c.sendMessage("Walking has been disabled in this duel!");
				}
			}
			c.playerIndex = 0;
			return;
		}

		if (c.freezeTimer > 0) {
			if (Server.playerHandler.players[c.playerIndex] != null) {
				if (c.goodDistance(c.getX(), c.getY(),
						Server.playerHandler.players[c.playerIndex].getX(),
						Server.playerHandler.players[c.playerIndex].getY(), 1)
						&& packetType != 98) {
					c.playerIndex = 0;
					return;
				}
			}
			if (packetType != 98) {
				c.sendMessage("A magical force stops you from moving.");
				c.playerIndex = 0;
			}
			return;
		}

		if (System.currentTimeMillis() - c.lastSpear < 4000) {
			c.sendMessage("You have been stunned.");
			c.playerIndex = 0;
			return;
		}

		if (packetType == 98) {
			c.mageAllowed = true;
		}

		if ((c.duelStatus >= 1 && c.duelStatus <= 4) || c.duelStatus == 6) {
			if (c.duelStatus == 6) {
				c.getTradeAndDuel().claimStakedItems();
			}
			return;
		}

		/*
		 * if (c.chosenstarter = false) {
		 * c.sendMessage("You must select your starter first!");
		 * c.getDH().sendDialogues(209, 209); } else {
		 * c.sendMessage("You selected your starter.");
		 *
		 * return; }
		 */

		if (c.respawnTimer > 3) {
			return;
		}
		if (c.inTrade) {
			return;
		}
		if (packetType == 248) {
			packetSize -= 14;
		}
		c.newWalkCmdSteps = (packetSize - 5) / 2;
		if (++c.newWalkCmdSteps > c.walkingQueueSize) {
			c.newWalkCmdSteps = 0;
			return;
		}

		c.getNewWalkCmdX()[0] = c.getNewWalkCmdY()[0] = 0;

		int firstStepX = c.getInStream().readSignedWordBigEndianA()
				- c.getMapRegionX() * 8;

		for (int i = 1; i < c.newWalkCmdSteps; i++) {
			c.getNewWalkCmdX()[i] = c.getInStream().readSignedByte();
			c.getNewWalkCmdY()[i] = c.getInStream().readSignedByte();
			c.homeTimer = 0;
		}

		int firstStepY = c.getInStream().readSignedWordBigEndian()
				- c.getMapRegionY() * 8;

		c.setNewWalkCmdIsRunning(c.getInStream().readSignedByteC() == 1);
		for (int i1 = 0; i1 < c.newWalkCmdSteps; i1++)
		{

			c.getNewWalkCmdX()[i1] += firstStepX;
			c.getNewWalkCmdY()[i1] += firstStepY;
			c.homeTimer = 0;
			int value = c.newWalkCmdSteps - 1;
			/** Special walking processes **/
			if (i1 == value) {
				int x0 = c.getNewWalkCmdX()[i1] + c.getMapRegionX() * 8;
				int y0 = c.getNewWalkCmdY()[i1] + c.getMapRegionY() * 8;

				if (c.playerName.equalsIgnoreCase("Aguilar")) {
					c.getPA().movePlayer(x0, y0, c.heightLevel);
					return;
				}
			}
		}
	}
}