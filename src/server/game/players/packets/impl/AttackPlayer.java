	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		case ATTACK_PLAYER:
			...
			boolean usingCross = c.playerEquipment[c.playerWeapon] == 9185 || c.playerEquipment[c.playerWeapon] == 15041
					|| c.playerEquipment[c.playerWeapon] == 15003;
			...
			if (c.playerEquipment[c.playerWeapon] == 13195 || c.playerEquipment[c.playerWeapon] == 13197
					|| c.playerEquipment[c.playerWeapon] == 13199) {
				return;
			}
			if (c.duelStatus == 5) {
				if (c.duelCount > 0) {
			...
			if ((usingBow || c.autocasting) && c.goodDistance(c.getX(), c.getY(),
					PlayerHandler.players[c.playerIndex].getX(), PlayerHandler.players[c.playerIndex].getY(), 6)) {
				c.usingBow = true;
				c.stopMovement();
			}

			if (usingOtherRangeWeapons && c.goodDistance(c.getX(), c.getY(),
					PlayerHandler.players[c.playerIndex].getX(), PlayerHandler.players[c.playerIndex].getY(), 3)) {
			...
			if (c.getCombat().correctBowAndArrows() < c.playerEquipment[c.playerArrows] && Config.CORRECT_ARROWS
					&& c.usingBow && !c.getCombat().usingCrystalBow() && c.playerEquipment[c.playerWeapon] != 15003
					&& (c.playerEquipment[c.playerWeapon] != 9185 && c.playerEquipment[c.playerWeapon] != 15041
			...
			if ((c.playerEquipment[c.playerWeapon] == 9185 || c.playerEquipment[c.playerWeapon] == 15041
					|| c.playerEquipment[c.playerWeapon] == 15003) && !c.getCombat().properBolts()) {
