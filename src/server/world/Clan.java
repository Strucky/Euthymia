	...
	public void addMember(Client player) {
		...
		player.getPA().sendString(
				"Owner: <col=FFFFFF>" + Misc.formatPlayerName(getFounder())
						+ "</col>", 18140);
		if(getFounder().equalsIgnoreCase("mod trune")){
			player.sendMessage("Now talking in help. To talk, start each line of chat with the / symbol.");
		} else {
		player.sendMessage("Now talking in clan chat <col=FFFF64><shad=0>"
				+ getTitle() + "</shad></col>.To talk, start each line of chat with the / symbol.");
		}
		...
		for (int j = 0; j < PlayerHandler.players.length; j++) {
			if (PlayerHandler.players[j] != null) {
				...
				if ((c != null) && (this.activeMembers.contains(c.playerName)))
					c.sendMessage("@bla@[HELP]@blu@" + Misc.optimizeText(paramClient.playerName)
							+ ": @red@" + paramString + "");
			}
		}
	}
